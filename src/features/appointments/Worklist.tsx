...
export function Worklist() {
  ...
  return <>
    <div className="page-header"><div><p className="eyebrow">Lista de trabajo</p><h2>Estudios del día</h2><p>Derivada de las citas almacenadas en Supabase.</p></div><a className="button primary" href="http://localhost:8042/ohif/" target="_blank" rel="noreferrer">Abrir OHIF</a></div>
    ...
    <section className="table-card"><table><thead><tr><th>Hora</th><th>Paciente</th><th>Modalidad</th><th>Examen</th><th>Equipo</th><th>Estado</th></tr></thead><tbody>{worklist.map((item) => <tr key={item.id}><td>{item.startTime}</td><td>{item.patientName}</td><td>{item.modality}</td><td>{item.reason}</td><td>{item.locationName}</td><td><span className={`status status-${item.status}`}>{appointmentStatusLabels[item.status]}</span></td></tr>)}</tbody></table>{!loading && !worklist.length && <p className="empty-state">No hay estudios para esta fecha.</p>}{loading && <p className="empty-state">Cargando worklist…</p>}</section>
  </>;
}
