...
export function PhrLaboratories({ documents, ownerUserId, readOnly, onUpload, hideHeading = false }: { documents: PortalDocument[]; ownerUserId: string; readOnly: boolean; onUpload: () => void; hideHeading?: boolean }) {
  ...
  return <div className="phr-labs-stack">
    ...
    {confirmed.length > 0 && <section className="card portal-card"><div className="card-heading"><div><p className="eyebrow">Resultados confirmados</p><h2>Historial de laboratorio</h2><p>Cada biomarcador aparece una vez. Ábrelo sólo cuando quieras ver sus mediciones anteriores y detalles técnicos.</p></div>{!readOnly && <div className="phr-actions"><button className="button secondary" type="button" onClick={() => void downloadPhrFile("/api/portal/trends", "tendencias-laboratorio.csv", "csv")}>Descargar CSV</button><button className="button secondary" type="button" onClick={() => void downloadPhrFile("/api/portal/trends", "tendencias-laboratorio.pdf", "pdf")}>Descargar PDF</button></div>}</div>
      ...
      {profiles.length ? <div className="phr-lab-profiles">{profiles.map((group) => <details className="phr-lab-profile" key={group.key} open={filter !== "all" || !!query.trim() || onlyOutside}><summary><span><strong>{labCategoryLabels[group.key]}</strong><small>{measurementExplanation(group.items[0])}</small></span><span>{group.markers.length} biomarcador(es) · {group.items.filter(outsideRange).length} fuera de rango</span></summary><div className="phr-lab-marker-list">{group.markers.map((history) => { const latest = history[0], favorite = favorites.some((item) => item.trendKey === phrLabTrendKey(latest)); return <details className="phr-lab-marker" key={phrLabTrendKey(latest)}><summary><span className="phr-result-title"><strong>{latest.analyte}</strong>{!readOnly && <button className="favorite-button" type="button" aria-label={favorite ? `Quitar ${latest.analyte} de favoritos` : `Agregar ${latest.analyte} a favoritos`} aria-pressed={favorite} onClick={(event) => { event.preventDefault(); event.stopPropagation(); void toggleFavorite(latest); }}>{favorite ? "★" : "☆"}</button>}</span><span className="phr-lab-latest"><strong>{resultValue(latest)}</strong><small>{shownDate(latest.observedAt)} · {history.length === 1 ? "1 medición" : `${history.length} mediciones`}</small></span><span className={`lab-flag lab-flag-${latest.flag || "none"}`}>{labFlagLabels[latest.flag]}</span></summary><div className="phr-lab-history">{history.map((result) => <article key={result.id}><div><strong>{shownDate(result.observedAt)}</strong><span>{phrSpecimenLabel(result.loincMetadata, result.sourceSentence)}{result.loincCode && ` · LOINC ${result.loincCode}`}</span></div><div><strong>{resultValue(result)}</strong><span>{[reference(result), reportedRangeLabel(result)].filter(Boolean).join(" · ")}</span></div></article>)}</div></details>; })}</div></details>)}</div> : <p className="empty-state">No encontramos resultados con esos filtros.</p>}
    ...
    {loincDialog && <div className="appointment-info-backdrop" role="dialog" aria-modal="true" aria-label="Sugerencias LOINC" onClick={(event) => { if (event.target === event.currentTarget && !busyId) setLoincDialog(undefined); }}><div className="appointment-info phr-loinc-dialog"><div className="card-heading"><div><p className="eyebrow">Homologación segura</p><h2>Revisar códigos LOINC</h2><p>El PDF no fue enviado. Las equivalencias verificadas aparecen preseleccionadas; la IA sólo ordena alternativas existentes y nunca asigna un código por sí sola.</p></div><button className="text-button" type="button" disabled={!!busyId} onClick={() => setLoincDialog(undefined)}>Cerrar</button></div>{loincDialog.warnings.map((warning) => <p className="form-notice" key={warning}>{warning}</p>)}<div className="phr-loinc-review-list">{loincDialog.suggestions.map((suggestion) => <label key={suggestion.resultId}><span><strong>{suggestion.analyte}</strong><small>{suggestion.currentCode ? `Actual: ${suggestion.currentCode}` : "Sin código actual"}{suggestion.recommendedCode && " · equivalencia verificada"}</small></span><select value={loincSelections[suggestion.resultId] ?? ""} disabled={!suggestion.options.length || !!busyId} onChange={(event) => setLoincSelections((current) => ({ ...current, [suggestion.resultId]: event.target.value }))}><option value="">Mantener sin asignar</option>{suggestion.options.map((option) => <option value={option.code} key={option.code}>{option.code} · {option.display}</option>)}</select></label>)}</div><button className="button primary" type="button" disabled={!!busyId || !Object.values(loincSelections).some(Boolean)} onClick={() => void applyLoinc()}>{busyId === "loinc" ? "Guardando…" : "Guardar equivalencias revisadas"}</button></div></div>}
  </div>;
}
