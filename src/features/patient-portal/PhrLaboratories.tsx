...
const resultCategory = (result: PhrLabResult) => phrSpecimenLabel(result.loincMetadata, result.sourceSentence) === "Orina" ? "urine" : labCategory({ analyte: phrLabDisplayName(result), loincMetadata: result.loincMetadata });
...
function SuggestedResult({ result, busy, selected, onSource, onDone, onError }: { result: PhrLabResult; busy: boolean; selected: boolean; onSource: (result: PhrLabResult) => Promise<void>; onDone: (message: string) => Promise<void>; onError: (message: string) => void }) {
  ...
  return <article className={`phr-lab-review-card${selected ? " selected" : ""}`}>
    {result.canonicalAnalyte && result.canonicalAnalyte !== result.analyte && <p className={`phr-canonical-suggestion${result.loincCode ? " matched" : " pending"}`}><strong>{result.loincCode ? "Homologado como" : "Posible homologación"}:</strong> {result.canonicalAnalyte}{confidence && ` · confianza ${confidence}`}</p>}
