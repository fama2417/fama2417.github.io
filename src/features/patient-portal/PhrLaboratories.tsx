...
export function PhrLaboratories({ documents, ownerUserId, readOnly, onUpload }: { documents: PortalDocument[]; ownerUserId: string; readOnly: boolean; onUpload: () => void }) {
  ...
  async function analyze(document: PortalDocument, rematch = false) {
    ...
    try {
      const response = await analyzePhrLabDocument(document.id, rematch); await reload();
      setNotice(`${rematch ? `Agrupación actualizada: ${response.loincMapped} resultado(s) con LOINC.` : response.duplicate ? "Este PDF ya estaba analizado." : `${response.created} resultado(s) listos para revisión.`}${response.warnings.length ? ` ${response.warnings.join(" ")}` : ""}`);
  ...
  const suggestedCategories = labCategoryOrder.map((key) => ({ key, items: suggested.filter((result) => labCategory({ analyte: result.analyte, loincMetadata: result.loincMetadata }) === key) })).filter((group) => group.items.length);
  const categories = labCategoryOrder.map((key) => ({ key, items: confirmed.filter((result) => labCategory({ analyte: result.analyte, loincMetadata: result.loincMetadata }) === key && (filter === "all" || filter === key)) })).filter((group) => group.items.length);
