...
function Onboarding({ onReady }: { onReady: (profile: PhrProfile) => void }) {
  ...
  return <main className="phr-app"><div className="portal-shell phr-onboarding"><section className="card portal-card"><PhrBrand /><p className="eyebrow">Bienvenido a Mi Salud</p><h1>Crea tu registro personal</h1><p>Este registro es tuyo y no depende de una clínica.</p><form className="phr-profile-form" onSubmit={submit}><ProfileFields /><label className="phr-consent"><input name="privacy" type="checkbox" required /> <span>Confirmo que soy mayor de 18 años y acepto la <a href="/portal/privacidad" target="_blank" rel="noreferrer">política de privacidad</a>.</span></label><button className="button primary" disabled={busy} type="submit">{busy ? "Creando…" : "Crear mi registro"}</button>{error && <p className="form-error" role="alert">{error}</p>}</form></section></div></main>;
}
