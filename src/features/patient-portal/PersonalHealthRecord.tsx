...
import {
  ...
  extractPhrImagingText, fetchPhrHealthItems, fetchPhrProfile, savePhrProfile, uploadPatientDocument, type PhrProfile, type PortalDocument,
} from "./repository";
...
import { inspectPatientPhoto, isPhrClinicalArea, PHR_CLINICAL_AREAS, PHR_CLINICAL_AREA_LABELS, documentTypeForSelection, suggestedPatientDocumentDate, suggestedPatientDocumentName } from "./documents";
...
function PhrBrand() {
  return <span className="phr-app-brand"><svg aria-hidden="true" viewBox="0 0 32 32" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M16 27S4 19.7 4 10.8C4 5.4 10.8 3.3 16 9c5.2-5.7 12-3.6 12 1.8C28 19.7 16 27 16 27Z" /><path d="M16 9v18" opacity=".35" /></svg><strong>Mi Salud</strong></span>;
}
...
function Onboarding({ onReady }: { onReady: (profile: PhrProfile) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await savePhrProfile({ fullName: String(form.get("fullName")), birthDate: String(form.get("birthDate")), identifier: String(form.get("identifier")), acceptedPrivacy: form.get("privacy") === "on", acceptedTerms: form.get("terms") === "on" }); const profile = await fetchPhrProfile(); if (!profile) throw new Error("No fue posible abrir tu registro."); onReady(profile); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "No fue posible crear tu registro."); } finally { setBusy(false); }
  }
  return <main className="phr-app"><div className="portal-shell phr-onboarding"><section className="card portal-card"><PhrBrand /><p className="eyebrow">Bienvenido a Mi Salud</p><h1>Crea tu registro personal</h1><p>Este registro es tuyo y no depende de una clínica.</p><form className="phr-profile-form" onSubmit={submit}><ProfileFields /><label className="phr-consent"><input name="privacy" type="checkbox" required /> <span>Confirmo que soy mayor de 18 años y consiento expresamente el tratamiento de mis datos personales y sensibles para las finalidades descritas en la <a href="/portal/privacidad" target="_blank" rel="noreferrer">política de privacidad</a>.</span></label><label className="phr-consent"><input name="terms" type="checkbox" required /> <span>Acepto los <a href="/portal/privacidad#terminos" target="_blank" rel="noreferrer">términos de uso</a>.</span></label><button className="button primary" disabled={busy} type="submit">{busy ? "Creando…" : "Crear mi registro"}</button>{error && <p className="form-error" role="alert">{error}</p>}</form></section></div></main>;
}
...
export function PersonalHealthRecord() {
  ...
  const [documents, setDocuments] = useState<PortalDocument[]>([]), [healthItems, setHealthItems] = useState<PhrHealthItem[]>([]), [tab, setTab] = useState<Tab>("inicio");
  ...
  const [imagingDraft, setImagingDraft] = useState<PhrImagingText>();
  ...
  useEffect(() => { if (ownerUserId && profiles?.some((profile) => profile.userId === ownerUserId)) Promise.all([fetchPatientDocuments(ownerUserId), fetchPhrHealthItems(ownerUserId)]).then(([nextDocuments, nextItems]) => { setDocuments(nextDocuments); setHealthItems(nextItems); }).catch(() => setError("No fue posible cargar el registro personal.")); }, [ownerUserId, profiles?.length]);
  useEffect(() => { const close = (event: KeyboardEvent) => { if (event.key === "Escape") { setAccountOpen(false); setMoreOpen(false); } }; window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close); }, []);
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const warning = await inspectPatientPhoto(file); if (warning && !window.confirm(`${warning}\n\n¿Subirla de todos modos?`)) return;
  ...
  async function remove(document: PortalDocument) { if (!window.confirm(`¿Eliminar ${document.filename}?`)) return; setBusy(true); try { await deletePatientDocument(document.id); const [nextDocuments, nextItems] = await Promise.all([fetchPatientDocuments(ownerUserId), fetchPhrHealthItems(ownerUserId)]); setDocuments(nextDocuments); setHealthItems(nextItems); setNotice("Documento eliminado."); } catch (cause) { setError(cause instanceof Error ? cause.message : "No fue posible eliminar el documento."); } finally { setBusy(false); } }
  ...
  if (profiles === undefined) return <main className="phr-app"><div className="portal-shell"><p className="phr-app-empty">Abriendo tu registro…</p></div></main>;
  ...
  if (!profile) return <main className="phr-app"><div className="portal-shell"><p className="phr-app-empty">No fue posible abrir el perfil seleccionado.</p></div></main>;
  ...
  return <main className="phr-app"><div className="phr-shell">
    <header className="phr-app-header">
      <PhrBrand />
      <nav className="phr-app-desktop-nav" aria-label="Secciones de Mi Salud">{mainTabs.map(([id, label]) => <button key={id} aria-current={tab === id ? "page" : undefined} type="button" onClick={() => navigate(id)}>{label}</button>)}</nav>
      <div className="phr-app-account">
        <button className="phr-app-account-button" type="button" aria-expanded={accountOpen} aria-controls="phr-account-menu" onClick={() => setAccountOpen((open) => !open)}><span className="phr-app-avatar" aria-hidden="true">{initials(profile.fullName)}</span><span>{profile.fullName.split(/\s+/)[0]}{readOnly && <small>Solo lectura</small>}</span><span aria-hidden="true">⌄</span></button>
        {accountOpen && <div className="phr-app-menu" id="phr-account-menu">{profiles.length > 1 && <label>Perfil<select value={ownerUserId} onChange={(event) => { setOwnerUserId(event.target.value); navigate("inicio"); }}>{profiles.map((item) => <option key={item.userId} value={item.userId}>{item.fullName}{item.userId === currentUserId ? " (yo)" : ""}</option>)}</select></label>}<button type="button" onClick={() => navigate("perfil")}>Perfil y seguridad</button><button type="button" onClick={() => supabase.auth.signOut()}>Cerrar sesión</button></div>}
      </div>
    </header>
    {error && <p className="notice danger-text" role="alert">{error}</p>}{notice && <p className="form-notice" role="status">{notice}</p>}
    {tab === "inicio" && <><section className="phr-app-hero"><div><h1>Hola, {profile.fullName.split(/\s+/)[0]}</h1><p>Tu información de salud, organizada para ti.</p><div className="phr-app-hero-actions">{!readOnly && <button className="phr-app-button primary" type="button" onClick={() => navigate("documentos")}>↑ <span>Subir resultado</span></button>}<button className="phr-app-button secondary" type="button" onClick={() => navigate("resumen")}>□ <span>Ver mi resumen</span></button></div></div><div className="phr-app-botanical" aria-hidden="true"><span /><span /><span /><span /></div></section><PhrDashboard ownerUserId={ownerUserId} documents={documents} healthItems={healthItems} onNavigate={navigate} /></>}
    {tab === "resumen" && <PhrHealthSummary profile={profile} items={healthItems} documents={documents} readOnly={readOnly} onChanged={async () => setHealthItems(await fetchPhrHealthItems(ownerUserId))} />}
    {tab === "documentos" && <>{!readOnly && <section className="card portal-card"><p className="eyebrow">Biblioteca personal</p><h2>Subir documento</h2><form className="portal-document-form" onSubmit={upload}><label className="wide-field">Archivo<input name="file" type="file" accept="application/pdf,image/jpeg,image/png,application/dicom,.dcm" capture="environment" required onChange={(event) => { const file = event.target.files?.[0], form = event.currentTarget.form; if (!file || !form) return; const name = form.elements.namedItem("displayName") as HTMLInputElement | null, date = form.elements.namedItem("documentDate") as HTMLInputElement | null; if (name && !name.value) name.value = suggestedPatientDocumentName(file.name); if (date && !date.value) date.value = suggestedPatientDocumentDate(file.lastModified); }} /><small>En el teléfono, encuadra la hoja completa, sin sombras y con el texto nítido.</small></label><label>Nombre del documento<input name="displayName" maxLength={240} placeholder="Ej.: Resonancia de rodilla" required /></label><label>Fecha del examen<input name="documentDate" type="date" required /><small>La sugerimos desde el archivo; confírmala antes de guardar.</small></label><label>Área o tipo<select name="clinicalArea" defaultValue="laboratory"><optgroup label="Resultados clínicos">{PHR_CLINICAL_AREAS.map((area) => <option value={area} key={area}>{PHR_CLINICAL_AREA_LABELS[area]}</option>)}</optgroup><optgroup label="Otros documentos"><option value="prescription">Receta u orden</option></optgroup></select></label><label>Institución de origen<input name="sourceInstitution" maxLength={160} required /></label><button className="button primary" disabled={busy} type="submit">{busy ? "Guardando y organizando…" : "Guardar documento"}</button></form></section>}<section className="card portal-card"><h2>Mis documentos</h2>{documentList(documents)}</section></>}
    {tab === "laboratorios" && <PhrResults documents={documents} ownerUserId={ownerUserId} readOnly={readOnly} onUpload={() => navigate("documentos")} onImaging={setImagingDocument} />}
    ...
    <nav className="phr-app-bottom-nav" aria-label="Navegación principal">
      {([["inicio", "⌂", "Inicio"], ["resumen", "♡", "Resumen"], ["laboratorios", "◷", "Resultados"], ["documentos", "□", "Documentos"]] as const).map(([id, icon, label]) => <button key={id} type="button" aria-current={tab === id ? "page" : undefined} onClick={() => navigate(id)}><span aria-hidden="true">{icon}</span>{label}</button>)}
    ...
    {imagingDocument && <PhrImagingTextDialog document={imagingDocument} initialText={imagingDraft} onClose={() => { setImagingDocument(undefined); setImagingDraft(undefined); }} onSaved={(text) => { setDocuments((current) => current.map((item) => item.id === imagingDocument.id ? { ...item, extractedText: text } : item)); setImagingDraft(undefined); setNotice("Informe organizado. Ya puedes ver la conclusión y la explicación simple."); }} />}
  </div></main>;
}
