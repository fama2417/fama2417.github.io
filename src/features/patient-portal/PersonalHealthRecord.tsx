...
import {
  ...
  fetchPhrHealthItems, fetchPhrProfile, savePhrProfile, uploadPatientDocument, type PhrProfile, type PortalDocument,
} from "./repository";
...
import { inspectPatientPhoto, isPhrClinicalArea, PHR_CLINICAL_AREAS, PHR_CLINICAL_AREA_LABELS, documentTypeForSelection } from "./documents";
...
export function PersonalHealthRecord() {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try { if (!(file instanceof File) || !file.name) throw new Error("Selecciona un archivo."); const warning = await inspectPatientPhoto(file); if (warning && !window.confirm(`${warning}\n\n¿Subirla de todos modos?`)) return; const selection = String(data.get("clinicalArea")); const documentType = documentTypeForSelection(selection) as PortalDocument["documentType"]; await uploadPatientDocument({ file, documentDate: String(data.get("documentDate")), documentType, clinicalArea: isPhrClinicalArea(selection) ? selection : "other", sourceInstitution: String(data.get("sourceInstitution")) }); setDocuments(await fetchPatientDocuments(ownerUserId)); form.reset(); setNotice("Documento guardado en tu registro personal."); }
  ...
  const documentList = (items: PortalDocument[]) => items.length ? <ul className="portal-list">{items.map((document) => <li key={document.id}><div className="portal-document-detail"><strong title={document.filename}>{friendlyDocumentName(document.documentType)}</strong><span>{document.documentType === "prescription" ? documentLabels.prescription : PHR_CLINICAL_AREA_LABELS[document.clinicalArea]} · {document.sourceInstitution} · {fileSize(document.sizeBytes)}</span><span>{shownDate(document.documentDate ?? document.createdAt)} · <span title={document.filename}>{document.filename}</span></span></div><div className="portal-document-actions"><a className="button secondary" href={document.url} target="_blank" rel="noreferrer" aria-label={`Abrir ${document.filename}`}>Abrir</a>{!readOnly && document.documentType === "imaging" && ["application/pdf", "image/jpeg", "image/png"].includes(document.mimeType) && <button className="button secondary" disabled={busy} type="button" onClick={() => setImagingDocument(document)}>{document.extractedText ? "Ver texto" : "Extraer texto"}</button>}{!readOnly && <button className="text-button danger-text" disabled={busy} type="button" onClick={() => void remove(document)}>Eliminar</button>}</div></li>)}</ul> : <p className="empty-state">Todavía no tienes documentos.</p>;
