...
export function PersonalHealthRecord() {
  ...
  return <main className="phr-app"><div className="phr-shell">
    ...
    {tab === "documentos" && <>{!readOnly && <section className="card portal-card"><p className="eyebrow">Biblioteca personal</p><h2>Subir documento</h2><form className="portal-document-form" onSubmit={upload}><label className="wide-field">Archivo<input name="file" type="file" accept="application/pdf,image/jpeg,image/png,application/dicom,.dcm" capture="environment" required onChange={(event) => { const file = event.target.files?.[0], name = event.currentTarget.form?.elements.namedItem("displayName") as HTMLInputElement | null; if (file && name && !name.value) name.value = suggestedPatientDocumentName(file.name); }} /><small>En el teléfono, encuadra la hoja completa, sin sombras y con el texto nítido.</small></label><label>Nombre del documento<input name="displayName" maxLength={240} placeholder="Ej.: Resonancia de rodilla" required /><small>Puedes cambiarlo antes de guardar.</small></label><label>Fecha del examen <span className="optional">Opcional</span><input name="documentDate" type="date" /><small>Si la dejas vacía, intentaremos detectarla al analizar el laboratorio.</small></label><label>Área o tipo<select name="clinicalArea" defaultValue="laboratory"><optgroup label="Resultados clínicos">{PHR_CLINICAL_AREAS.map((area) => <option value={area} key={area}>{PHR_CLINICAL_AREA_LABELS[area]}</option>)}</optgroup><optgroup label="Otros documentos"><option value="prescription">Receta u orden</option></optgroup></select></label><label>Institución de origen<input name="sourceInstitution" maxLength={160} required /></label><button className="button primary" disabled={busy} type="submit">{busy ? "Guardando y organizando…" : "Guardar documento"}</button></form></section>}<section className="card portal-card"><h2>Mis documentos</h2>{documentList(documents)}</section></>}
    {tab === "laboratorios" && <PhrResults documents={documents} ownerUserId={ownerUserId} readOnly={readOnly} onUpload={() => navigate("documentos")} onImaging={setImagingDocument} onDocumentDate={(documentId, date) => setDocuments((current) => current.map((document) => document.id === documentId ? { ...document, documentDate: date } : document))} />}
