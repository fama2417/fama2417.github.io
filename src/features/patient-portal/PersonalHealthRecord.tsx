...
export function PersonalHealthRecord() {
  ...
  return <main className="phr-app"><div className="phr-shell">
    ...
    <nav className="phr-app-bottom-nav" aria-label="Navegación principal">
      {([["inicio", "⌂", "Inicio"], ["laboratorios", "◷", "Resultados"], ["documentos", "□", "Documentos"]] as const).map(([id, icon, label]) => <button key={id} type="button" aria-current={tab === id ? "page" : undefined} onClick={() => navigate(id)}><span aria-hidden="true">{icon}</span>{label}</button>)}
