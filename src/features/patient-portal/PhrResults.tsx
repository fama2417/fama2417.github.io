...
export function PhrResults({ documents, ownerUserId, readOnly, onUpload, onImaging }: { documents: PortalDocument[]; ownerUserId: string; readOnly: boolean; onUpload: () => void; onImaging: (document: PortalDocument) => void }) {
  ...
  return <div className="phr-results-stack">
    ...
    {area === "laboratory" ? <PhrLaboratories documents={documents} ownerUserId={ownerUserId} readOnly={readOnly} onUpload={onUpload} hideHeading /> : <section className="card portal-card"><p className="eyebrow">{PHR_CLINICAL_AREA_LABELS[area]}</p><h2>Informes y documentos</h2>{selected.length ? <ul className="portal-list">{selected.map((document) => <li key={document.id}><div className="portal-document-detail"><strong>{document.filename}</strong><span>{document.sourceInstitution} · {shownDate(document.documentDate ?? document.createdAt)}</span></div><div className="portal-document-actions"><a className="button secondary" href={document.url} target="_blank" rel="noreferrer">Abrir archivo</a>{area === "imaging" && !readOnly && ["application/pdf", "image/jpeg", "image/png"].includes(document.mimeType) && <button className="button primary" type="button" onClick={() => onImaging(document)}>{document.extractedText ? "Ver texto" : "Extraer texto"}</button>}</div></li>)}</ul> : <p className="empty-state">Todavía no tienes resultados de {PHR_CLINICAL_AREA_LABELS[area].toLocaleLowerCase("es-CL")}.</p>}</section>}
  </div>;
}
