...
function ImagingResults({ documents, readOnly, onImaging }: { documents: PortalDocument[]; readOnly: boolean; onImaging: (document: PortalDocument) => void }) {
  ...
  return <div className="phr-labs-stack">
    ...
    {!!organized.length && <section className="card portal-card"><p className="eyebrow">Paso 2 · Consultar</p><h2>Mis informes organizados</h2><p>La conclusión se conserva tal como fue informada. La explicación simple aclara términos sin agregar diagnósticos.</p><div className="phr-imaging-list">{organized.map((document) => <article key={document.id}><div className="phr-imaging-card-heading"><div><h3>{document.filename}</h3><span>{document.sourceInstitution || "Institución no informada"} · {shownDate(document.documentDate ?? document.createdAt)}</span></div><span className="status-badge">{document.extractedText?.plainLanguage ? "Explicación lista" : "Texto organizado"}</span></div>{document.extractedText?.impression ? <div className="phr-imaging-card-impression"><span>Conclusión original</span><p>{document.extractedText.impression}</p></div> : <p className="phr-measurement-help">El documento fue organizado, pero no traía una conclusión separada.</p>}{document.extractedText?.plainLanguage && <div className="phr-imaging-plain"><span>En palabras simples · redactado por IA</span><ul>{phrImagingPlainLanguagePoints(document.extractedText.plainLanguage).map((point, index) => <li key={`${document.id}-${index}`}>{point}</li>)}</ul><small>Esta explicación no reemplaza la evaluación de un profesional de salud.</small></div>}<div className="phr-actions"><a className="button secondary" href={document.url} target="_blank" rel="noreferrer">Abrir original</a>{!readOnly && imagingMime(document) && <button className="button secondary" type="button" onClick={() => onImaging(document)}>Revisar extracción</button>}</div></article>)}</div></section>}
  </div>;
}
...
export function PhrResults({ documents, ownerUserId, readOnly, onUpload, onImaging, onDocumentDate }: { documents: PortalDocument[]; ownerUserId: string; readOnly: boolean; onUpload: () => void; onImaging: (document: PortalDocument) => void; onDocumentDate?: (documentId: string, date: string) => void }) {
  ...
  return <div className="phr-results-stack">
    ...
    {area === "laboratory" ? <PhrLaboratories documents={documents} ownerUserId={ownerUserId} readOnly={readOnly} onUpload={onUpload} onDocumentDate={onDocumentDate} hideHeading /> : area === "imaging" ? <ImagingResults documents={selected} readOnly={readOnly} onImaging={onImaging} /> : <section className="card portal-card"><p className="eyebrow">{PHR_CLINICAL_AREA_LABELS[area]}</p><h2>Informes y documentos</h2>{selected.length ? <ul className="portal-list">{selected.map((document) => <li key={document.id}><div className="portal-document-detail"><strong>{document.filename}</strong><span>{document.sourceInstitution} · {shownDate(document.documentDate ?? document.createdAt)}</span></div><div className="portal-document-actions"><a className="button secondary" href={document.url} target="_blank" rel="noreferrer">Abrir archivo</a></div></li>)}</ul> : <p className="empty-state">Todavía no tienes resultados de {PHR_CLINICAL_AREA_LABELS[area].toLocaleLowerCase("es-CL")}.</p>}</section>}
  </div>;
}
