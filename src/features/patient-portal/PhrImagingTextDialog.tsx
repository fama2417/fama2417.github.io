...
export function PhrImagingTextDialog({ document, onClose, onSaved }: { document: PortalDocument; onClose: () => void; onSaved: (text: PhrImagingText) => void }) {
  ...
  const [operation, setOperation] = useState<"extract" | "save" | "">(document.extractedText ? "" : "extract");
  ...
  return <div className="appointment-info-backdrop" role="dialog" aria-modal="true" aria-label="Informe radiológico estructurado" onClick={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <div className="appointment-info phr-imaging-text-dialog">
      ...
      {!busy && !text ? <div className="empty-state"><p>No hay texto organizado para revisar.</p><button className="button primary" type="button" onClick={() => void extract()}>Intentar nuevamente</button></div> : text && !editing ? <div className="phr-imaging-report">
        ...
        <div className="phr-imaging-fields"><p><strong>Paso 2 de 3 · Revisar.</strong> Compara cada sección con el original. La explicación asistida también debe ser confirmada antes de guardarse.</p>
