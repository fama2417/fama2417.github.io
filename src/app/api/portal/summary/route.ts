...
export async function GET(request: NextRequest) {
  ...
  const [documents, labResults] = await Promise.all([
    ...
    auth.db.from("phr_lab_results").select("id, loinc_code, analyte, value_num, value_text, unit, flag, observed_at").eq("owner_user_id", auth.user.id).eq("review_status", "confirmed").order("observed_at", { ascending: false }),
  ]);
  if (documents.error || labResults.error) return NextResponse.json({ error: "No fue posible preparar el resumen." }, { status: 500 });
  ...
  const lines = [
    ...
    p.emergency_notes ? `Notas: ${p.emergency_notes}` : "", "", "ÚLTIMOS RESULTADOS FUERA DEL RANGO INFORMADO POR EL LABORATORIO",
    ...(outside.length ? outside.map((result) => `${result.observed_at} | ${result.analyte}: ${result.value_num ?? result.value_text} ${result.unit}`) : ["No hay resultados confirmados fuera del rango informado."]),
