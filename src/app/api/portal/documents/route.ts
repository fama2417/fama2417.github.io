...
import { hasValidPatientDocumentSignature, validatePatientDocument } from "@/features/patient-portal/documents";
...
export async function POST(request: NextRequest) {
  ...
  if (!hasValidPatientDocumentSignature(bytes, file.type)) return NextResponse.json({ error: "El contenido no corresponde a un PDF, JPG o PNG válido." }, { status: 400 });
  ...
  if (uploaded.error) return NextResponse.json({ error: "No fue posible guardar el archivo." }, { status: 500 });
  const inserted = await db.from("patient_documents").insert({
    ...
    size_bytes: file.size, document_date: documentDate || null, document_type: documentType, source_institution: sourceInstitution,
  });
  if (inserted.error) {
