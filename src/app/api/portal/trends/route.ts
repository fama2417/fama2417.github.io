...
import { phrLabDisplayName, type PhrLabResult } from "@/features/patient-portal/labs";
...
export async function GET(request: NextRequest) {
  ...
  const query = await auth.db.from("phr_lab_results").select("id, document_id, loinc_code, analyte, canonical_analyte, value_num, value_text, unit, ref_low, ref_high, ref_text, flag, observed_at, source, review_status")
  ...
  const results = (query.data ?? []).map((row) => ({
    id: row.id, documentId: row.document_id, loincCode: row.loinc_code, analyte: row.analyte, canonicalAnalyte: row.canonical_analyte,
  ...
  const lines = results.length ? results.map((result) => `${result.observedAt} | ${phrLabDisplayName(result)}: ${result.valueNum ?? result.valueText} ${result.unit} | ${reportedRangeLabel(result)}`) : ["No hay resultados confirmados."];
