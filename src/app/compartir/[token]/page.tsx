...
export default async function SharedPhrPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [documents, results] = await Promise.all([
    ...
    resultIds.length ? db.from("phr_lab_results").select("id, analyte, canonical_analyte, value_num, value_text, unit, ref_low, ref_high, ref_text, flag, observed_at").in("id", resultIds).eq("review_status", "confirmed").order("observed_at", { ascending: false }) : Promise.resolve({ data: [] }),
  ]);
